...
\begin{tikzpicture}
    \matrix (m) [
        ...
        Count per group & |[align=left]| \texttt{SELECT g, COUNT(*) FROM T GROUP BY g} \\
        Top N per group & |[align=left]| \texttt{WITH ranked AS (...) ROW\_NUMBER() OVER} \\
        Self-join & |[align=left]| \texttt{FROM T t1, T t2 WHERE t1.x = t2.y} \\
        NOT IN pattern & |[align=left]| \texttt{WHERE x NOT IN (SELECT x FROM ...)} \\
    };
\end{tikzpicture}
\end{center}
