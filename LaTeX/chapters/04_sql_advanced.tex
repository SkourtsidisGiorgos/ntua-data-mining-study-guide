...
\begin{examplebox}[RANK vs DENSE\_RANK vs ROW\_NUMBER]
\textbf{Data:} Scores = [100, 100, 90, 80]

\begin{center}
\begin{tabular}{clll}
\toprule
\textbf{Score} & \textbf{ROW\_NUMBER} & \textbf{RANK} & \textbf{DENSE\_RANK} \\
\midrule
100 & 1 & 1 & 1 \\
100 & 2 & 1 & 1 \\
90 & 3 & 3 & 2 \\
...
\begin{itemize}
    \item \texttt{ROW\_NUMBER}: Always unique, arbitrary for ties
    \item \texttt{RANK}: Same rank for ties, \textbf{skips} next ranks (gap after tie)
    \item \texttt{DENSE\_RANK}: Same rank for ties, \textbf{no gaps}
\end{itemize}
...
\begin{tikzpicture}
    ...
    \node[draw=accent, fill=box-red, rounded corners=5pt, minimum width=5cm, minimum height=1.5cm]
        at ([xshift=2cm, yshift=-0.5cm]main.south) {};
    \node[align=center, font=\footnotesize] at ([xshift=2cm, yshift=-0.5cm]main.south) {
